...
myFun:
        mult    $a1, $t1            
        mflo    $t2                 
        add     $a0, $a0, $t2       # addition becomes subtraction
        addi    $t9, $t9, 1
        slt     $t0, $a0, $a1       
        beq     $t0, $zero, myFun   # Loops to beginning
done:
        jal     find_gcd
        move    $t4, $v0
        beq     $a0, $zero, skip
...
ras:
        ...
        add     $t5, $zero, $a1     
myLoop:
        beq     $t4, $t5, done2     # begin while(a0 != a1)
        beq     $t4, $zero, done2   
        
        sub     $t2, $t4, $t5      
        ### ABS VAL PART
        slt     $t0, $t2, $zero     #  t0 = t2 < 0
        beq     $t0, $zero, skipAbs # if $t0 eq zero, goto
        mult    $t2, $t3            # t2 * (-1) if t2 is neg
        mflo    $t2                 # move result of mult into t2
        ### ABS VAL PART
skipAbs:
        slt     $t0, $t5, $t4       # t0 = num2 < num1
        bne     $t0, $zero, set     # if(t0 != 0) goto set
        add     $t5, $zero, $t2     # num2 = d
        j       myLoop

set:
        add     $t4, $zero, $t2     # num1 = d
        j       myLoop
